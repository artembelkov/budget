...
// Apply rules (in order) to a description. Returns {category, vendor} or null.
...
function categorize(description) {
  ...
  if (hit) return hit;
  return { category: 'Uncategorized', vendor: guessVendor(description) };
}
...
function renderDashboard() {
  const list = periodFilter(transactions);
...
function renderTransactions() {
  ...
  tbody.innerHTML = pageItems.map(t => `
    <tr data-id="${t.id}">
      ...
      <td class="num ${t.amount < 0 ? 'amount-neg' : 'amount-pos'}">${fmtMoney(t.amount)}</td>
      <td>
      ...
      <td><button class="icon-btn" data-rule-from="${t.id}" title="Create a rule from this transaction">＋ rule</button></td>
    </tr>`).join('');
...
document.querySelector('#txTable tbody').addEventListener('change', e => {
  const sel = e.target.closest('select.row-category');
  if (!sel) return;
  const t = transactions.find(t => t.id === sel.dataset.id);
  if (!t) return;
...
document.getElementById('reapplyRulesBtn').addEventListener('click', () => {
  ...
  transactions.forEach(t => {
    if (t.manualOverride) return;
    const hit = applyRules(t.description);
    if (hit && (hit.category !== t.category || hit.vendor !== t.vendor)) {
...
document.getElementById('ruleForm').addEventListener('submit', e => {
  ...
  if (!match || !vendor || !category) return;
  rules.push({ match, vendor, category });
  persistRules();
  e.target.reset();
  renderRules();

  // offer to apply immediately to existing uncategorized matches
  let applied = 0;
  transactions.forEach(t => {
    if (t.manualOverride) return;
    if (t.description.toLowerCase().includes(match.toLowerCase())) {
      t.category = category;
      t.vendor = vendor;
      applied++;
    }
