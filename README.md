...
- **Vendor rules** — teach Ledger that any description containing, say,
  ...
  existing transactions at any time. A rule can also **exclude** matching
  transactions from all totals and charts entirely — useful for transfers
  between your own accounts, or anything that isn't real income or spending.
- **Manual editing** — change a transaction's category, amount, or
  include/exclude status directly from the Transactions table. Manual
  choices are locked in and won't be overwritten by rules later.
