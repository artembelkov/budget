...
uses (Chart.js for charts, PapaParse for CSV parsing) are vendored in the
`vendor/` folder rather than loaded from a CDN, so the site works fully
offline and isn't affected by ad blockers or firewalls blocking third-party
scripts. Just open `index.html` in a browser, or serve the folder locally:
...
vendor/      — self-hosted copies of Chart.js and PapaParse (no CDN calls)
```

No framework, no bundler, no build step. The two vendored libraries are the
full, unmodified minified builds — to upgrade either one later, download the
newer version and replace the matching file in `vendor/`.
